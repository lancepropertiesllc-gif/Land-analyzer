...
BALLWIN_CENTER = [38.5953, -90.5454]  # fallback only
TILE_OPTIONS = {
...
def build_map(
    ...
    # Calculate center from actual data if not overridden
    if center == BALLWIN_CENTER and not df.empty:
        valid = df.dropna(subset=["LATITUDE","LONGITUDE"])
        valid = valid[
            valid["LATITUDE"].between(36.0, 42.0) &
            valid["LONGITUDE"].between(-96.0, -89.0)
        ]
        if not valid.empty:
